...
server.post('/api/create-key', (req, res) => {
...
// Custom route for purchase/upload
server.get('/api/unlocks/:username', (req, res) => {
  const username = req.params.username;
  const db = router.db;
  const unlocks = db.get('unlocks').filter({ username: username }).value();
  res.json(unlocks);
});

// Custom route for purchase/upload
server.get('/api/purchases/:username', (req, res) => {
  const username = req.params.username;
  const db = router.db;
  const purchases = db.get('buyCredits').filter({ username: username }).value();
  res.json(purchases);
});

// Custom route for purchase/upload
server.get('/api/redemptions/:username', (req, res) => {
  const username = req.params.username;
  const db = router.db;
  const purchases = db.get('redeemCredits').filter({ username: username }).value();
  res.json(purchases);
});
